pub fn count_beam_splits(input: String) -> u64 {
    ...
    let mut visited = vec![vec![false; splitter_matrix[0].len()]; splitter_matrix.len()];
    follow_bean(start_point,splitter_matrix, &mut visited)
}
...
fn follow_bean(point: (i32, i32), splitter_matrix: Vec<Vec<bool>>, visited: &mut Vec<Vec<bool>>) -> u64 {
    if point.0 < 0 || point.1 <= 0
        || point.0 as usize >= splitter_matrix.len()
    ...
    if is_splitter {
        ...
        follow_bean((point.0 + 1, point.1),splitter_matrix, visited)
    }
}
