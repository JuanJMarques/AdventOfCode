...
fn main() {
    ...
    println!("all cells flashed at step: {}", calculate_total_flashes(contents));
}

fn calculate_total_flashes(contents: String) -> u32 {
    let mut state = parse_initial_state(contents);
    let mut step = 0;
    let mut all_cells_flashed = false;
    while !all_cells_flashed {
        ...
        all_cells_flashed = flashed_cells_table.iter()
                .fold(true, |acc, x| acc && x.iter().all(|y| *y));
        step += 1;
    }
    step
}
