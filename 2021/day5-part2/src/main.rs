...
fn main() {
    ...
    for (mut origin, end) in vents {
        let mut x_direction = 0;
        if origin.0 != end.0 {
            x_direction = (end.0 - origin.0) / (origin.0 - end.0).abs();
        }
        let mut y_direction = 0;
        if origin.1 != end.1 {
            y_direction = (end.1 - origin.1) / (origin.1 - end.1).abs();
        }
        while origin.0 != end.0 || origin.1 != end.1 {
            grid[origin.1 as usize][origin.0 as usize] += 1;
            origin.0 += x_direction;
            origin.1 += y_direction;
        }
        grid[origin.1 as usize][origin.0 as usize] += 1;
    }
