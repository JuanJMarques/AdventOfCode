...
fn expand_grid(grid: Vec<Vec<u8>>, size: u8) -> Vec<Vec<u8>> {
    let mut new_grid = vec![vec![0; grid[0].len() * size as usize]; grid.len() * size as usize];
    for i in 0..grid.len() {
        for j in 0..grid[0].len() {
            for k in 0..size {
                let mut new_value = grid[i][j] + k;
                new_value = new_value.div(10) + new_value % 10;
                new_grid[i][j + grid[0].len() * k as usize] = new_value;
                new_grid[i + grid.len() * k as usize][j] = new_value;
            }
        }
    }
    for i in 0..grid.len() {
        for j in 0..new_grid[0].len() {
            for k in 1..size {
                let mut new_value = new_grid[i][j] + k;
                new_value = new_value.div(10) + new_value % 10;
                new_grid[i + grid.len() * k as usize][j] = new_value;
            }
        }
