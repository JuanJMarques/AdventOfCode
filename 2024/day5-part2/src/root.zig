...
pub fn swapSort(pageElements: std.ArrayList([]const u8), indexMap: std.StringHashMap(usize), rulesMap: std.StringHashMap(std.ArrayList([]const u8))) !std.AutoHashMap(usize, []const u8) {
    const allocator = std.heap.page_allocator;
    var inverseIndex = std.AutoHashMap(usize, []const u8).init(allocator);
    var pagesIt = indexMap.keyIterator();
    while (pagesIt.next()) |pagePointer| {
        const pageKey = pagePointer.*;
        try inverseIndex.put(indexMap.get(pageKey).?, pageKey);
    }
    var i : usize = 0;
    while (i<pageElements.items.len-1): (i+=1) {
        var j: usize = i+1;
        while (j<pageElements.items.len): (j+=1) {
            const prev = inverseIndex.get(i).?;
            const current = inverseIndex.get(j).?;
            if (rulesMap.contains(current)) {
                for (rulesMap.get(current).?.items) |currentPageRule| {
                    if (std.mem.eql(u8, prev, currentPageRule)) {
                        try inverseIndex.put(i, current);
                        try inverseIndex.put(j, prev);
                    }
                }
            }
        }
...
test "test case" {
    ...
    for (result.items) |line| {
        ...
        if (cleanLine.len == 0) {
            ...
            if (parsingRules) {
                ...
                if (!validPage) {
                    var inverseIndexMap = try swapSort(pageElements, indexMap, currentRules);
                    defer inverseIndexMap.deinit();
                    const middleElement = inverseIndexMap.get(pageElements.items.len / 2).?;
    ...
    try testing.expectEqual(123, total);
}