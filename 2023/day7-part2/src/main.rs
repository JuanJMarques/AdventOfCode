...
fn calculate_hand_strength(cards: &str) -> u32 {
    ...
    if is_five_of_a_kind(&cards, 0) {
        return 7;
    }
    if is_n_of_a_kind(&cards, 4, 0) {
        return 6;
    }
    if is_full_house(&cards, 0) {
        return 5;
    }
    if is_n_of_a_kind(&cards, 3, 0) {
        return 4;
    }
    if is_two_pairs(&cards, 0) {
        return 3;
    }
    if is_n_of_a_kind(&cards, 2, 0) {
        return 2;
    }
    1
}
...
fn is_five_of_a_kind(cards: &[char], i: usize) -> bool {
    if i == 5 {
        return true;
    }
    let reference = cards[i];
    if reference == 'J' {
        return is_five_of_a_kind(cards, i + 1);
    }
    cards
        .iter()
        .filter(|&&s| s == reference || s == 'J')
        .count()
        == 5
}

fn is_n_of_a_kind(cards: &[char], n: usize, i: usize) -> bool {
    if cards.len() < n {
        return false;
    }
    let reference = cards[i];
    if reference == 'J' {
        return is_n_of_a_kind(cards, n, i + 1);
    }
    if cards
        .iter()
        .filter(|&&s| s == reference || s == 'J')
...
fn is_full_house(cards: &[char], i: usize) -> bool {
    let reference = cards[i];
    if reference == 'J' {
        return is_full_house(cards, i + 1);
    }
    if is_n_of_a_kind_strict(cards, 3, reference) {
        return is_n_of_a_kind(
            &cards
                .iter()
                .filter(|&&c| c != reference && c != 'J')
                .copied()
                .collect::<Vec<char>>(),
            2,
            0,
...
fn is_two_pairs(cards: &[char], i: usize) -> bool {
    if cards.len() < 4 {
        return false;
    }
    let reference = cards[i];
    if reference == 'J' {
        return is_two_pairs(cards, i + 1);
    }
    if is_n_of_a_kind_strict(cards, 2, reference) {
        return is_n_of_a_kind(
            &cards
